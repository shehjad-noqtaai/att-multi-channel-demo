...
// The gallery is published-perspective like every other route. When no
// release has been promoted yet, the list is intentionally empty.
  ...
  return (
    <div className="page page--wide">
      <header className="gallery__intro">
        ...
        <p>
          Pick an offer and a persona to see how the same campaign brief is rendered for
          different audiences.
        </p>
      </header>
      ...
      {offers.length === 0 ? (
        <p style={{color: 'var(--color-muted)'}}>
          No published offers yet. Generate variations in the Studio and publish the
          release to see them here.
        </p>
      ) : (
        <div className="gallery__grid">
          {offers.map((o) => {
            const firstPersona = o.personas[0]
            const accent = (firstPersona && getPersona(firstPersona)?.brandColor) || PERSONAS.new.brandColor
            return (
              <article
                key={o._id}
